...
export function StoreProfileForm() {
  ...
  if (isLoading) {
    return <div className="space-y-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-10 animate-pulse rounded-lg bg-gray-100" />)}</div>
  }

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 max-w-lg">
      <div className="space-y-1.5">
        <Label htmlFor="store-name">Nama Toko <span className="text-red-500">*</span></Label>
        <Input id="store-name" {...register('name')} className={errors.name ? 'border-red-500' : ''} />
      ...
      <div className="space-y-1.5">
        ...
        <Input id="store-email" type="email" {...register('email')} className={errors.email ? 'border-red-500' : ''} placeholder="toko@email.com" />
