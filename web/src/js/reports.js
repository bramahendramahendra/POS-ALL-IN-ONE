...
function formatPaymentMethod(method) {
  const MAP = { cash: 'Cash', debit: 'Debit', credit: 'Kredit', transfer: 'Transfer', qris: 'QRIS' };
...
async function filterSalesReport() {
  ...
  try {
    const res = await apiClient.get('/reports/sales', {
      ...
      end_date: endDate,
      user_id: document.getElementById('salesUserFilter').value || undefined,
      payment_method: document.getElementById('salesPaymentFilter').value || undefined
    });
    if (!res || !res.success) {
      Toast.error((res && res.message) || 'Gagal memuat laporan');
      return;
    }
    salesData = res;
    salesCurrentPage = 1;
...
function renderSalesSummary(summary) {
  document.getElementById('salesTotalAmount').textContent      = formatCurrency(summary?.total_sales || 0);
  document.getElementById('salesTotalTransactions').textContent = (summary?.total_transactions || 0).toLocaleString('id-ID');
  document.getElementById('salesAvgTransaction').textContent   = formatCurrency(summary?.avg_per_transaction || 0);
  document.getElementById('salesTotalItems').textContent        = (summary?.total_items_sold || 0).toLocaleString('id-ID');
}
...
function renderSalesTable() {
  ...
  document.getElementById('salesTableInfo').textContent = `${total.toLocaleString('id-ID')} transaksi`;
  ...
  if (totalPages > 1) {
    ...
    document.getElementById('salesPageInfo').textContent = `Halaman ${salesCurrentPage} / ${totalPages}`;
...
function setupPaginationHandlers() {
  ...
  document.getElementById('salesNextPage').addEventListener('click', () => {
    const totalPages = Math.ceil((salesData.transactions?.length || 0) / ROWS_PER_PAGE);
...
async function filterPLReport() {
  ...
  try {
    ...
    if (!res || !res.success) {
      Toast.error((res && res.message) || 'Gagal memuat laporan laba rugi');
      return;
    }
    plData = res;
...
function renderExpensePieChart(expenseCategories) {
  ...
  const PALETTE = ['#3498db','#e74c3c','#2ecc71','#f39c12','#9b59b6','#1abc9c','#e67e22','#34495e','#e91e63','#00bcd4'];
...
async function filterStockReport() {
  ...
  try {
    const res = await apiClient.get('/reports/stock', {
      category_id:  document.getElementById('stockCategoryFilter')?.value || undefined,
      stock_status: document.getElementById('stockStatusFilter')?.value || undefined
    });
    if (!res || !res.success) {
      Toast.error((res && res.message) || 'Gagal memuat laporan stok');
      return;
    }
    stockData = res;
...
function renderStockSummary(summary) {
  document.getElementById('stockTotalSKU').textContent        = (summary?.total_sku || 0).toLocaleString('id-ID');
  document.getElementById('stockInventoryValue').textContent  = formatCurrency(summary?.total_inventory_value || 0);
  document.getElementById('stockActiveProducts').textContent  = (summary?.active_products || 0).toLocaleString('id-ID');
  document.getElementById('stockLowCount').textContent        = (summary?.low_stock_count || 0).toLocaleString('id-ID');
...
function renderStockTable(products) {
  ...
  document.getElementById('stockTableInfo').textContent = `${(products || []).length.toLocaleString('id-ID')} produk`;
  ...
  tbody.innerHTML = products.map(p => {
    const statusClass = p.stock_status === 'empty' ? 'badge-danger' : p.stock_status === 'low' ? 'badge-warning' : 'badge-success';
    const statusText  = p.stock_status === 'empty' ? 'Habis' : p.stock_status === 'low' ? 'Menipis' : 'Aman';
    const rowClass    = p.stock_status === 'empty' ? 'row-danger' : p.stock_status === 'low' ? 'row-warning' : '';
...
async function filterCashierReport() {
  ...
  const userId = document.getElementById('cashierUserFilter').value || null;
  ...
  try {
    ...
    if (!res || !res.success) {
      Toast.error((res && res.message) || 'Gagal memuat laporan kasir');
      return;
    }
    cashierData = res;
    ...
    await loadCashierShiftBreakdown(startDate, endDate, userId);
  } catch (e) {
...
function renderCashierBarChart(stats) {
  ...
  cashierChartInstance = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: { callbacks: { label: ctx => 'Rp ' + ctx.parsed.x.toLocaleString('id-ID') } }
      },
      scales: {
        x: { beginAtZero: true, ticks: { callback: v => 'Rp ' + (v / 1000).toFixed(0) + 'rb' } }
      }
    }
  });
}
...
async function loadCashierShiftBreakdown(startDate, endDate, userId) {
  ...
  try {
    ...
    if (!result || !result.success || !result.summary.length) {
      container.innerHTML = '';
      return;
    }
    ...
    const sessions = (cashResult && cashResult.success) ? cashResult.history : [];
    ...
    sessions.forEach(s => {
      ...
      if (!byShift[key]) {
        byShift[key] = {
          ...
          sessions: 0, total_cash_sales: 0, total_expenses: 0, total_difference: 0
        };
      }
...
function exportSalesPDF() {
  if (!salesData.transactions?.length) { Toast.warning('Tidak ada data untuk di-export'); return; }
  try {
    ...
    doc.autoTable({
      head: [['Tanggal', 'Kode', 'Kasir', 'Customer', 'Items', 'Total', 'Metode']],
      body: salesData.transactions.map(t => [
...
function printSalesReport() {
  if (!salesData.transactions?.length) { Toast.warning('Tidak ada data untuk di-print'); return; }
  window.print();
}

function exportPLPDF() {
  if (plData.summary?.revenue === undefined) {
...
function exportStockExcel() {
  if (!stockData.products?.length) { Toast.warning('Tidak ada data untuk di-export'); return; }
  try {
    const { utils, writeFile } = window.XLSX;
    ...
    const rows = stockData.products.map(p => [
      p.barcode, p.name, p.category_name, p.stock, p.min_stock,
      p.stock_status === 'empty' ? 'Habis' : p.stock_status === 'low' ? 'Menipis' : 'Aman',
      p.purchase_price, p.inventory_value, p.unit || 'pcs'
    ]);
...
function exportCashierPDF() {
  ...
  if (!filtered.length) { Toast.warning('Tidak ada data untuk di-export'); return; }
  try {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF();
    ...
    doc.autoTable({
      ...
      body: sorted.map((s, i) => [
        i + 1, s.full_name, s.username, s.total_transactions,
        formatCurrency(s.total_sales), formatCurrency(s.avg_per_transaction)
      ]),
      startY: 32,
      styles: { fontSize: 9 },
